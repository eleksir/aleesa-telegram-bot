...
!lat | !лат                 - сгенерить фразу из крылатых латинских выражений
...
sub kitty {
	...
	if ($r->{success}) {
		my $jcat = eval { decode_json ($r->{content}) };

		if ($jcat->[0]->{url}) {
			my @cats = ('龴ↀ◡ↀ龴', '=^..^=', '≧◔◡◔≦ ','^ↀᴥↀ^' );
			return sprintf '[%s](%s)', $cats[int(rand($#cats))], $jcat->[0]->{url};
		} else {
			return 'Нету кошечек, все разбежались.';
		}
	} else {
		return 'Нету кошечек, все разбежались.';
	}
